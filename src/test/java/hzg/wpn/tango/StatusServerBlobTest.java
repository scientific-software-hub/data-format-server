...
public class StatusServerBlobTest {
    ...
    public void testWrite() throws Exception {
        PipeBlob testBlob = new PipeBlobBuilder("status_server")
                .add("int-blob", new PipeBlobBuilder("int-blob").add("nxPath", "/entry/long").add("values", new int[]{1, 2, 3}, false).add("times", new long[]{100L, 101L, 100L}).build())
        ...
        verify(mockFile, atLeastOnce()).write("/entry/long/value", 1, true);
        verify(mockFile, atLeastOnce()).write("/entry/long/value", 2, true);
        verify(mockFile, atLeastOnce()).write("/entry/long/value", 3, true);
